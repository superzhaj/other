...
// from C++ Concurrency In Action Second Edition
// lock-free queue implementation by helping out another thread
...
private:
	...
	std::atomic<counted_node_ptr> head;    // ptr to the 1st data node
	std::atomic<counted_node_ptr> tail;    // ptr to the node after the last data node
	...
	void push(T new_value)
	{
		...
		for (;;)
		{
			...
			if (old_tail.ptr->data.compare_exchange_strong(old_data, new_data.get()))
			{
				...
				if (!old_tail.ptr->next.compare_exchange_strong(old_next, new_next)) // if "next" is set by other thread
				{
					delete new_next.ptr;
					new_next = old_next;
				}
			...
			{
				counted_node_ptr old_next = { 0 };
				if (old_tail.ptr->next.compare_exchange_strong(old_next, new_next))  // set "next" for "if" thread
				{
					old_next = new_next;
					new_next.ptr = new node;
				}
